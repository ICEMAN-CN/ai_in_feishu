...
| 测试编号 | 测试场景 | 预期结果 | 验证方法 |
|---------|---------|---------|---------|
| TC-5.6-001 | 单文件夹同步 | 文档入库 | 单元测试 |
| TC-5.6-002 | 单文档同步 | chunk+embedding+存储 | 单元测试 |
| TC-5.6-003 | 语义检索 | 返回相关结果 | 单元测试 |
...
| 状态 | ✅ 完成 |
| Commit | `9761736` |
| 实现文件 | `src/services/rag-pipeline.ts`, `src/core/vector-store.ts` |
| 测试文件 | `tests/rag-pipeline.test.ts`, `tests/vector-store.test.ts` |
| 测试用例 | TC-5.6-001 ~ TC-5.6-003 (3个) + 边界测试 (12个) = 15个 |
| API 端点 | `POST /api/admin/kb/sync`, `GET /api/admin/kb/stats` |
...
| 5.2 文档拉取 | [x] | 能获取文档列表和内容 |
| 5.3 文档分块 | [x] | 按配置分块 |
| 5.4 Embedding | [x] | 向量生成正确 |
| 5.5 向量存储 | [x] | LanceDB读写正常 |
| 5.6 检索服务 | [x] | 语义检索返回结果 |
