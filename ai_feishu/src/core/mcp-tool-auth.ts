...
const DEFAULT_TOOLS: ToolAuthConfig[] = [
...
export class MCPToolAuthManager {
  constructor(private db: Database.Database, private mcpClient?: MCPClient) {
    ...
    for (const tool of DEFAULT_TOOLS) {
      ...
      if (!existing) {
        try {
          this.db
            .prepare(
              'INSERT OR IGNORE INTO mcp_tool_auth (tool_name, enabled, fallback_enabled, fallback_method, updated_at) VALUES (?, ?, ?, ?, ?)'
            )
            .run(
              tool.toolName,
              tool.enabled ? 1 : 0,
          ...
          this.db
            .prepare(
              'INSERT OR IGNORE INTO mcp_tool_auth (tool_name, enabled, fallback_enabled, fallback_method) VALUES (?, ?, ?, ?)'
            )
            .run(
              tool.toolName,
              tool.enabled ? 1 : 0,
  ...
  getAllToolAuths(): ToolAuthConfig[] {
    const rows = this.db.prepare('SELECT * FROM mcp_tool_auth').all() as any[];
    return rows.map((row) => ({
      toolName: row.tool_name,
      enabled: row.enabled === 1,
      fallbackEnabled: row.fallback_enabled === 1,
  ...
  async callToolIfAllowed(toolName: string, args: Record<string, unknown>): Promise<unknown> {
