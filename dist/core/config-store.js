...
export function initDatabase() {
  ...
  `);
    ...
    const version = database.prepare('SELECT MAX(version) as v FROM schema_version').get();
...
export function saveModel(model) {
  ...
  `);
    stmt.run(model.id, model.name, model.provider, model.apiKeyEncrypted, model.baseUrl, model.modelId, model.isDefault ? 1 : 0, model.maxTokens, model.temperature, model.enabled ? 1 : 0, model.createdAt, model.updatedAt);
...
export function getSessionByP2P(p2pId) {
...
export function saveKBFolder(folder) {
  ...
  `);
    stmt.run(folder.id, folder.name, folder.url, folder.folderToken, folder.lastSyncAt || null, folder.lastSyncDocCount, folder.syncEnabled ? 1 : 0, folder.createdAt);
}
export function deleteKBFolder(id) {
    getDb().prepare('DELETE FROM kb_folders WHERE id = ?').run(id);
}
function mapRowToKBFolder(row) {
    return {
        id: row.id,
        name: row.name,
        url: row.url,
...
// ==================== MCP Tool Auth ====================
export function getMCPToolAuth(toolName) {
    const row = getDb().prepare('SELECT * FROM mcp_tool_auth WHERE tool_name = ?').get(toolName);
    return row ? mapRowToMCPToolAuth(row) : null;
}
export function getAllMCPToolAuths() {
    const rows = getDb().prepare('SELECT * FROM mcp_tool_auth').all();
    return rows.map((row) => mapRowToMCPToolAuth(row));
}
export function saveMCPToolAuth(auth) {
    const stmt = getDb().prepare(`
    ...
    ON CONFLICT(tool_name) DO UPDATE SET
      ...
      updated_at = excluded.updated_at
  `);
    stmt.run(auth.toolName, auth.enabled ? 1 : 0, auth.fallbackEnabled ? 1 : 0, auth.fallbackMethod || null, new Date().toISOString());
}
function mapRowToMCPToolAuth(row) {
    return {
...
export function getAllSystemConfig() {
    const rows = getDb().prepare('SELECT key, value FROM system_config').all();
...
export function saveMessage(message) {
  ...
  `);
    stmt.run(message.id, message.sessionId, message.role, message.content, message.modelId || null, message.messageId || null, message.createdAt);
}
export function getMessagesBySession(sessionId, limit) {
...
export function deleteMessagesBySession(sessionId, keepCount = 0) {
    if (keepCount <= 0) {
  ...
  `).all(sessionId, keepCount, sessionId);
