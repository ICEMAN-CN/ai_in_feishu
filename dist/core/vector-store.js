...
export async function searchChunks(_queryVector, topK = 5, _filterFolderId) {
    const store = await getVectorStore();
    try {
...
export async function getStats() {
    ...
    const docIds = new Set(all.map((r) => r.doc_id));
    const folderIds = new Set(all.map((r) => r.folder_id));
    return {
        totalChunks: chunks,
