...
export class RAGPipeline {
    ...
    chunkingService;
    embeddingService;
    constructor(folderManager, docService, chunkingService, embeddingService) {
    ...
    async syncDocument(documentId, title, folderId) {
    ...
    async retrieve(query, topK = MAX_RETRIEVAL_CHUNKS) {
        ...
        const formatted = results.map(r => `[来源: ${r.docTitle}](${r.docUrl})\n${r.textChunk}`).join('\n\n---\n\n');
